...
Expired makeup is more than a nuisance — it's a health risk. Bacteria builds up in opened cosmetics over time, leading to skin irritation, breakouts, and eye infections. Most products have a small PAO (Period After Opening) symbol on the packaging indicating how long they're safe to use, but it's easy to ignore and impossible to remember across a full collection.
...
| Preset PAO periods per category | Reduces cognitive load. Users don't need to research expiry times — the app knows mascara is 3 months and powder is 24 months. |
...
| Google OAuth (no custom auth) | A single sign-in flow gives us both user identity and calendar access. No passwords to manage, no separate permissions prompt. |
...
Existing apps like Beauty Keeper and Cosmetic Tracker offer similar expiry tracking, but they require users to check a separate app to see reminders. Makeup Expiry Reminder differentiates by integrating directly with Google Calendar — putting reminders where users already look every day — and by being web-based with zero download friction.
